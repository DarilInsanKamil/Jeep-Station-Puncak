...
<div>
    <form action="?/create"  method="POST" enctype="multipart/form-data">
        <div>
            ...
            <input type="file" name="gambar_bundle" placeholder="nama paket" onchange={handleChangeImage} required>
            {#if previewUrl}
                <div class="mt-4">
        ...
        <div>
            ...
            <input type="text" name="title" placeholder="nama paket" value={form?.values?.title}>
        </div>
        <div>
            <label for='harga'>
                Harga Paket Bundle
            </label>
            <input type="text" name="harga" placeholder="harga paket" value={form?.values?.harga}>
        </div>
        <div>
            <label for='jumlah_unit'>
                Jumlah Unit
            </label>
            <input type="text" name="jumlah_unit" placeholder="nama paket" defaultValue="2" value={form?.values?.jumlah_unit}>
        </div>
        <div>
            <label for='deskripsi'>
                Deskripsi
